...
\usepackage{multicol}
\usepackage{float}
\usepackage[a4paper, left=3.3cm, top=2cm, right=2cm, bottom=2cm]{geometry}
...
\pbr
UTF-8 is compatibel met ASCII, % TODO: finish
\subsection{Protocol}
...
% square bcs larger errors would have a bigger error
\pbr
...
% mathematica string data = {{23, 3213}, {21, 2089}, {25, 2253}, {18, 1801}, {13, 801}, {16, 1934}, {13, 1720}, {17, 1514}, {12, 1017}}
\begin{center}
  \begin{tabular}{|c|c|c|}
    \hline
...
% ListPlot[data, PlotTheme->"Scientific",FrameLabel->{{HoldForm[HoldForm[Inkomsten]],None},{HoldForm[Temperatuur],None}},PlotLabel->None, ImageSize->Full]
\begin{figure}[H]
  \centering
...
% ListPlot[data, PlotTheme->"Scientific", GridLines->{{158/9}, {16342/9}}, GridLinesStyle->{{Gray, Dashed}, {Gray, Dashed}},FrameLabel->{{HoldForm[HoldForm[Inkomsten]],None},{HoldForm[Temperatuur],None}},PlotLabel->None, ImageSize->Full]
\begin{wrapfigure}{l}{0.6\textwidth}
  \centering
  \includegraphics[width=0.55\textwidth]{RegressieCafeScatterplotAverage.jpg}
  \caption{$\overbar{y}$ en $\overbar{x}$ grafisch voorgesteld}
\end{wrapfigure}
We zouden bv het gemiddelde kunnen nemen. Het gemiddelde van onze inkomsten (y) is dan gelijk aan $\langle y \rangle = \frac{1}{n} \sum_{i=1}^ny_i}$
\lbr
In dit geval zou dat bij benadering 1815.78 zijn. Dit kunnen we ook grafisch gaan voorstellen met de simpele formule $y = \langle y \rangle$ we zullen deze lijn $\overbar{y}$ noemen. Dus $\overbar{y} = \langle y \rangle$. Soortgelijk gaan we het gemiddelde van de $x$ waarden $\overbar{x}$ noemen. Grafisch geeft ons dit.
\pbr\lbr
We hadden een evenredig verband verwacht dus het gemiddelde, $\overbar{y}$ stelt onze data eigenlijk niet zo heel goed voor. Maar $\overbar{y}$ en $\overbar{x}$ zal later nog wel van pas komen als we ons model verder gaan analyzeren voor nu kunnen we verder toewerken naar ons model.
\pbr
In ons voorbeeld willen we de inkomsten kunnen voorspellen gebaseerd op een gegeven temperatuur. De inkomsten is dus de afhankelijke variable, deze zullen we $y$ noemen en de temperatuur is onze onafhankelijke variable deze zullen we $x$ noemen. Gezien we maar 1 onafhankelijke variable hebben kunnen we volgende formule opstellen voor het ideale model van onze data, dit is.
\begin{ceqn}
  \begin{align}
  ...
  \end{align}
\end{ceqn}
We maken gebruik van griekse letters wanneer we theoretisch werken in de statistiek. Hier stelt $\beta_0$ het snijpunt met de y-as voor, $\beta_1$ stelt de helling van onze curve voor en $\epsilon$ is de fout van ons model. Deze laatste term moet erbij want hoe goed ons model ook is er is altijd een willekeurige fout waar ons model niet op kon voorzien zijn. $\beta_0$ en $\beta_1$ worden ook wel eens populatie parameters genoemt.
\pbr
...
% Show[ListPlot[data, PlotTheme->"Scientific", GridLines->{{158/9}, {16342/9}}, GridLinesStyle->{{Gray, Dashed}, {Gray, Dashed}},FrameLabel->{{HoldForm[HoldForm[Inkomsten]],None},{HoldForm[Temperatuur],None}},PlotLabel->None, ImageSize->Full], Plot[-153966/1395+18272/155 x, {x, 11,26}]]
\begin{figure}[H]
  \centering
